...
# Hardcoded safe fallback baseline
...
# Global client initialization using GEMINI_API_KEY environment variable
_client: Optional[genai.Client] = None

def get_genai_client() -> Optional[genai.Client]:
    """Retrieves or lazily initializes the official google-genai client."""
    global _client
    if _client is None:
...
def extract_queue_intent(user_text: str, tenant_persona: str = "general service desk") -> Dict[str, Any]:
    """
    Extracts structured demographic, urgency, and operational features from unstructured user text
    using Gemini Structured Outputs with Pydantic.

    Args:
        user_text: The customer's raw input description.
        tenant_persona: Context string defining the specific business type (e.g., 'bank', 'hospital', 'DMV').

    Returns:
        A dictionary representation of QueueIntent (or SAFE_FALLBACK if API/validation fails).
    """
    if not user_text or not user_text.strip():
        logger.info("Empty user text received; returning safe fallback.")
    ...
    try:
        ...
        # If text returned in JSON format
        if response and response.text:
            validated = QueueIntent.model_validate_json(response.text)
            return validated.model_dump()

        logger.warning("Gemini returned empty parsed response; falling back.")
        return SAFE_FALLBACK.copy()

    except Exception as exc:
        logger.error(f"Error during Gemini intent extraction ({exc}); using graceful fallback.", exc_info=True)
        return SAFE_FALLBACK.copy()

if __name__ == "__main__":
    # Quick self-test demonstration
    sample_text = "I rushed here with my elderly mother who is experiencing severe dizziness and shortness of breath."
    print("Testing intent extraction with sample text:")
    print(f"Input: '{sample_text}'")
    result = extract_queue_intent(user_text=sample_text, tenant_persona="hospital emergency intake")
