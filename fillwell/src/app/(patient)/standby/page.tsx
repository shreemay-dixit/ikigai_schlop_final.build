...
export default function StandbyPage() {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-between bg-stone-50 p-4 sm:p-6">
      {/* Header */}
      <div className="flex w-full max-w-lg items-center justify-between border-b border-stone-200 pb-3">
        <div className="flex items-center gap-2">
          <div className="rounded-lg bg-rose-600 p-2 text-white"><BrainCircuit className="h-4 w-4" /></div>
          <div>
            <h1 className="text-sm font-bold text-stone-900">Fillwell Patient Gateway</h1>
            <p className="text-[10px] text-stone-500">Gemini AI Triage & Live Standby</p>
          </div>
        </div>
        <div className="flex items-center gap-1.5 rounded-full border border-stone-200 bg-white px-2.5 py-1 text-[10px] font-mono text-stone-600 shadow-sm">
      ...
      <div className="my-auto w-full max-w-lg py-6">
        {/* ── Intake ── */}
        {state === "intake" && (
          <div className="animate-page-in rounded-2xl border border-stone-200 bg-white p-6 shadow-sm space-y-5">
            <div>
              <span className="inline-flex items-center gap-1 rounded-full bg-rose-50 border border-rose-100 px-2.5 py-1 text-[11px] font-semibold text-rose-700"><Sparkles className="h-3 w-3" />Step 1: AI Triage</span>
              <h2 className="mt-2 text-xl font-bold text-stone-900">Describe Your Symptoms</h2>
              <p className="text-xs text-stone-500">Gemini AI evaluates urgency and queues you on the live standby radar.</p>
            </div>
                ...
                <div>
                  ...
                  <div className="relative"><User className="pointer-events-none absolute left-2.5 top-2.5 h-4 w-4 text-stone-400" />
                    <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="Jane Doe"
                      className="w-full rounded-lg border border-stone-200 bg-stone-50 py-2 pl-8 pr-3 text-xs outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative"><Phone className="pointer-events-none absolute left-2.5 top-2.5 h-4 w-4 text-stone-400" />
                    <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+1 (555) 000-0000"
                      className="w-full rounded-lg border border-stone-200 bg-stone-50 py-2 pl-8 pr-3 text-xs font-mono outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100" />
                  </div>
                </div>
              </div>
              <div>
                <label className="mb-1 flex items-center justify-between text-xs font-medium text-stone-700">
                  <span className="flex items-center gap-1"><Stethoscope className="h-3 w-3 text-rose-600" />Chief Complaint</span>
                  <span className="text-[10px] text-rose-500 font-mono">Gemini 1.5</span>
                </label>
                <textarea rows={3} required value={symptoms} onChange={(e) => setSymptoms(e.target.value)} placeholder="Duration, severity, pain location…"
                  className="w-full rounded-lg border border-stone-200 bg-stone-50 p-3 text-xs outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100" />
              </div>
              <div className="grid grid-cols-2 gap-1.5">
                {PRESETS.map((c, i) => (
                  <button key={i} type="button" onClick={() => setSymptoms(c.text)}
                    className="truncate rounded-lg border border-stone-200 bg-white px-2 py-1.5 text-left text-[11px] text-stone-600 shadow-sm transition hover:border-rose-300 hover:text-rose-700">{c.label}</button>
                ))}
              </div>
              <button type="submit" className="flex w-full items-center justify-center gap-2 rounded-lg bg-rose-600 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-500">
        ...
        {/* ── Analyzing ── */}
        {state === "analyzing" && (
          <div className="animate-page-in rounded-2xl border border-stone-200 bg-white p-8 shadow-sm text-center space-y-5">
            <BrainCircuit className="mx-auto h-12 w-12 text-rose-600 animate-pulse" />
            <h2 className="text-lg font-bold text-stone-900">Running Gemini AI Triage…</h2>
            <p className="text-xs text-stone-500">Extracting symptoms, scoring urgency, prioritizing queue position.</p>
        ...
        {/* ── Waiting ── */}
        {state === "waiting" && (
          <div className="animate-page-in rounded-2xl border border-stone-200 bg-white p-6 shadow-sm space-y-6">
            {triage && (
              <div className="rounded-lg border border-stone-200 bg-stone-50 p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1 text-[10px] font-bold uppercase text-rose-600"><BrainCircuit className="h-3 w-3" />AI Triage</span>
                ...
                <p className="text-sm font-semibold text-stone-900">{triage.service_type || "Consultation"}</p>
                <p className="text-[11px] italic text-stone-500">"{triage.conversational_reply}"</p>
              </div>
            )}
            <div className="relative mx-auto flex h-32 w-32 items-center justify-center">
              <div className="absolute inset-0 animate-ping rounded-full bg-rose-500/10" />
              <div className="absolute inset-3 animate-pulse rounded-full bg-rose-500/20" />
              <div className="relative flex h-16 w-16 items-center justify-center rounded-full bg-rose-600 text-white shadow-lg">
                <Radio className="h-7 w-7 animate-bounce" />
              </div>
            </div>
            <div className="text-center">
              <h2 className="text-lg font-bold text-stone-900">Live Standby Active</h2>
              <p className="text-xs text-stone-500">Waiting for <strong>{name}</strong>. Keep this page open.</p>
            </div>
            <button onClick={() => { localStorage.removeItem("fillwell_standby_user"); setState("intake"); }}
              className="w-full rounded-lg border border-stone-200 bg-white py-2 text-xs font-medium text-stone-500 transition hover:bg-stone-50 hover:text-stone-800">← Edit Symptoms</button>
          </div>
        )}
        ...
        {state === "slot_opened" && (
          <div className="animate-page-in rounded-2xl border-2 border-amber-400 bg-amber-50 p-6 shadow-lg space-y-5 text-center">
            ...
            <div className="rounded-xl border border-amber-200 bg-white p-4 text-left text-xs space-y-1.5 shadow-sm">
              <div className="flex justify-between text-stone-500"><span>Service:</span><span className="font-bold text-amber-700">{slot?.service_type || "Consultation"}</span></div>
              <div className="flex justify-between text-stone-500"><span>Time:</span><span className="font-bold text-stone-900 font-mono">{new Date(slot?.start_time || Date.now()).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} Today</span></div>
            </div>
        ...
        {state === "claimed" && (
          <div className="animate-page-in rounded-2xl border-2 border-emerald-400 bg-white p-8 shadow-sm space-y-5 text-center">
            ...
            <h2 className="text-2xl font-extrabold text-stone-900">You Got It! 🎉</h2>
            <p className="text-sm text-stone-500">Appointment locked for <strong>{name}</strong>.</p>
            <button onClick={() => { localStorage.removeItem("fillwell_standby_user"); setState("intake"); }}
              className="w-full rounded-lg bg-stone-100 py-2.5 text-sm font-medium text-stone-700 transition hover:bg-stone-200">New Intake</button>
          </div>
        )}
        ...
        {state === "missed" && (
          <div className="animate-page-in rounded-2xl border-2 border-red-300 bg-white p-8 shadow-sm space-y-5 text-center">
            ...
            <h2 className="text-2xl font-extrabold text-stone-900">Missed It</h2>
            <p className="text-sm text-stone-500">{claimErr || "Another patient claimed it first."}</p>
            <button onClick={() => setState("waiting")}
              className="flex w-full items-center justify-center gap-2 rounded-lg bg-rose-600 py-2.5 text-sm font-semibold text-white transition hover:bg-rose-500">
      ...
      {/* Footer */}
      <div className="flex w-full max-w-lg items-center justify-center gap-1.5 border-t border-stone-200 pt-3 text-[10px] text-stone-400">
