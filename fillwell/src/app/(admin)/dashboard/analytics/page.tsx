...
export default function AnalyticsDashboard() {
  ...
  const Stat = ({ icon: Icon, label, value, trend, trendUp, color, href }: any) => (
    <div className="group relative overflow-hidden rounded-xl border border-slate-200 bg-white p-5 shadow-sm hover:shadow-md transition-all">
      <div className={`absolute -right-6 -top-6 h-32 w-32 rounded-full ${color.includes('emerald') ? 'bg-emerald-500' : color.includes('indigo') ? 'bg-indigo-500' : color.includes('amber') ? 'bg-amber-500' : 'bg-slate-500'} opacity-10 blur-2xl transition-transform duration-500 group-hover:scale-125`} />
      <div className="relative flex items-center justify-between z-10">
        <div className={`flex h-10 w-10 items-center justify-center rounded-xl shadow-inner ${color}`}>
          <Icon className="h-5 w-5" />
        </div>
        {href && (
          <Link href={href} className="text-[11px] font-bold uppercase tracking-wider text-slate-400 hover:text-indigo-600 flex items-center gap-1 transition-colors">
      ...
      <div className="relative mt-5 z-10">
        <p className="text-4xl font-black tracking-tight text-slate-900 tabular-nums">
          {loading ? <span className="inline-block h-9 w-20 animate-pulse rounded bg-slate-100" /> : <AnimatedCounter value={value} />}
        </p>
  ...
  return (
    <div className="space-y-8 max-w-7xl pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 flex items-center gap-2"><BarChart2 className="h-6 w-6 text-indigo-600" />Business Analytics</h1>
          <p className="text-sm text-slate-500">Real-time telemetry and revenue performance.</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleSeed} disabled={seeding} className="inline-flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-2 text-xs font-bold text-indigo-700 shadow-sm transition hover:bg-indigo-100 disabled:opacity-50">
            {seeding ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4 text-amber-500" />}
            Seed Sandbox Data
          </button>
          <button onClick={handleReset} disabled={seeding} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-600 shadow-sm transition hover:bg-slate-50 disabled:opacity-50">
            <RefreshCw className="h-4 w-4 text-slate-400" /> Factory Reset
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat icon={Users} label="Waitlisted" value={stats.waitlist} trend="4%" trendUp={true} color="bg-gradient-to-br from-indigo-500 to-blue-600 text-white" href="/dashboard" />
        ...
        <Stat icon={Calendar} label="Bookings" value={stats.appointments} trend="2%" trendUp={false} color="bg-gradient-to-br from-slate-700 to-slate-900 text-white" href="/dashboard/appointments" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-sm font-bold text-slate-900">24h Queue Volume (Simulated)</h2>
            <div className="flex gap-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-indigo-500" /> Triage</span>
          ...
          <div className="flex-1 flex items-end gap-2 h-48 relative">
            {/* Y-Axis lines */}
            <div className="absolute inset-0 flex flex-col justify-between border-l border-slate-100 pl-2">
              {[100, 75, 50, 25, 0].map(v => <span key={v} className="text-[9px] text-slate-300 font-mono -ml-6">{v}</span>)}
            </div>
            {/* CSS Bar Chart */}
            {Array.from({ length: 24 }).map((_, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col justify-end gap-0.5 group relative">
                  <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none whitespace-nowrap">
                    Hour {i}: {h1} Triaged
                  </div>
                  ...
                  <div className="w-full bg-indigo-500 rounded-b-sm transition-all duration-700 ease-out hover:brightness-110" style={{ height: `${h1}%` }} />
                </div>
              );
            })}
          </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col">
          <h2 className="text-sm font-bold text-slate-900 mb-1">Patient Standby Gateway</h2>
          <p className="text-[11px] text-slate-500 mb-5">
            QR link for patients to join the live standby waitlist via Gemini AI triage.
          </p>
          <div className="flex flex-col items-center flex-1 justify-center border-2 border-dashed border-slate-100 rounded-xl p-4 bg-slate-50/50 hover:bg-slate-50 transition-colors">
            <div className="rounded-xl border border-slate-200 p-4 bg-white shadow-sm mb-4">
              {portalUrl && <QRCodeSVG value={portalUrl} size={130} level="H" />}
            </div>
            <div className="w-full flex flex-col gap-2">
              <Link href="/standby" target="_blank" className="w-full inline-flex justify-center items-center gap-1.5 rounded-lg bg-indigo-600 px-4 py-2.5 text-xs font-semibold text-white shadow-sm hover:bg-indigo-500 transition">
                Open Standby Portal <ChevronRight className="h-4 w-4" />
              </Link>
              <Link href="/book" target="_blank" className="w-full inline-flex justify-center items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-700 shadow-sm hover:bg-slate-50 transition">
                Open Booking Portal
              </Link>
            </div>
          </div>
