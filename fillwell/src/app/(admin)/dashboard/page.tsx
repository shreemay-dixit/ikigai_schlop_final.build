  ...
  const urgencyBadge = (tier: string) => {
    ...
    return (
      <div className="group relative inline-block">
        ...
        <div className="pointer-events-none absolute bottom-full left-1/2 -transtone-x-1/2 mb-2 w-48 opacity-0 transition-opacity group-hover:opacity-100 bg-stone-900 text-white text-[10px] rounded p-2 text-center shadow-lg z-50">
          {m.tooltip}
          <div className="absolute top-full left-1/2 -transtone-x-1/2 border-4 border-transparent border-t-stone-900" />
        </div>
      </div>
    );
  };
  ...
  const columns: ColumnDef<WL>[] = useMemo(() => [
    {
      ...
      cell: ({ row }) => (
        <div>
          <p className="font-semibold text-stone-900">{row.original.patient_name}
            {!row.original.is_active && <span className="ml-1.5 rounded bg-stone-100 px-1.5 py-0.5 text-[10px] font-bold text-stone-400 uppercase border border-stone-200">Inactive</span>}
          </p>
    ...
    { accessorKey: "urgency_tier", header: "AI Triage", cell: ({ row }) => urgencyBadge(row.original.urgency_tier) },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <span className="rounded-md border border-stone-200 bg-stone-50 px-2 py-0.5 font-mono text-xs font-bold text-stone-700">{row.original.priority_score}/5</span>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <span className={`text-xs font-mono font-bold ${mins > 30 ? "text-red-600" : "text-stone-600"}`}>{mins}m elapsed</span>;
      },
    },
    { accessorKey: "notes", header: "Notes", cell: ({ row }) => <span className="line-clamp-1 max-w-[120px] text-[11px] text-stone-500 cursor-help" title={row.original.notes || ""}>{row.original.notes || "—"}</span> },
    {
      id: "actions", header: "",
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={() => handleBump(id)} disabled={actionId === id} title="Bump priority"
              className="rounded-md border border-rose-200 p-1.5 text-rose-600 transition hover:bg-rose-600 hover:text-white disabled:opacity-50 shadow-sm bg-rose-50">
            ...
            <button onClick={() => handleDelete(id)} disabled={actionId === id} title="Remove"
              className="rounded-md border border-stone-200 p-1.5 text-stone-500 transition hover:bg-red-50 hover:border-red-200 hover:text-red-600 disabled:opacity-50 shadow-sm bg-white">
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 max-w-[1400px]">
      ...
      <div className="flex-1 space-y-4">
        {/* Live System Ticker */}
        <div className="flex items-center gap-3 rounded-lg border border-stone-200 bg-white px-4 py-2 text-xs font-mono shadow-sm overflow-hidden whitespace-nowrap">
          ...
          <span className="text-stone-500">Active Engine: <span className="text-rose-600">Gemini 1.5 Triage</span></span>
          <span className="text-stone-300">|</span>
          <span className="text-stone-500">Realtime Sink: <span className="text-rose-600">Connected</span></span>
          <span className="text-stone-300">|</span>
          <div className="flex items-center gap-1 text-stone-500 animate-[pulse_3s_ease-in-out_infinite]">Last update: {new Date().toLocaleTimeString()}</div>
        </div>

        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="flex items-center gap-2 text-2xl font-bold tracking-tight text-stone-900"><Users className="h-6 w-6 text-rose-600" />Live Queue</h1>
            <p className="text-sm text-stone-500">Central command for priority waitlist and automated triage.</p>
          </div>
          <div className="relative w-full sm:w-auto">
            ...
            <input value={globalFilter ?? ""} onChange={(e) => setGlobalFilter(e.target.value)} placeholder="Search patient…"
              className="w-full sm:w-64 rounded-lg border border-stone-200 bg-white py-2 pl-9 pr-3 text-sm outline-none shadow-sm transition focus:border-rose-400 focus:ring-2 focus:ring-rose-100" />
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-stone-100">
                {loading ? Array.from({ length: 7 }).map((_, i) => (
                  <tr key={i}>{columns.map((_, j) => <td key={j} className="px-4 py-2"><div className="h-4 w-full animate-pulse rounded bg-stone-100" /></td>)}</tr>
                )) : table.getRowModel().rows.length ? table.getRowModel().rows.map((row) => (
                  <tr key={row.id} className={`transition-all duration-200 group ${row.original.is_active ? "hover:bg-rose-50/30 hover:shadow-inner" : "opacity-50"} ${row.original.priority_score >= 4 ? "bg-red-50/10" : ""}`}>
                  ...
                  <tr><td colSpan={columns.length} className="py-16 text-center text-stone-400">Queue is currently empty.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <div className="w-full lg:w-80 shrink-0 space-y-4">
        {/* Wait Time Card */}
        <div className="rounded-xl border border-stone-200 bg-stone-900 p-6 text-white shadow-lg relative overflow-hidden group">
          ...
          <h2 className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-stone-400 relative z-10"><Activity className="h-3.5 w-3.5 text-rose-400" />Queue Insights</h2>
          
          <div className="mt-6 space-y-6 relative z-10">
            <div>
              <p className="text-[10px] font-semibold text-stone-400 uppercase tracking-widest mb-1">Avg Wait Time</p>
            ...
            <div className="space-y-3 pt-4 border-t border-stone-800">
              <p className="text-[10px] font-semibold text-stone-400 uppercase tracking-widest">Priority Breakdown</p>
              <div className="flex items-center justify-between group/row hover:bg-stone-800 p-1 rounded transition-colors">
                <span className="flex items-center gap-2 text-sm text-stone-300"><span className="h-2 w-2 rounded-full bg-red-400 shadow-[0_0_8px_rgba(248,113,113,0.6)]" /> Urgent</span>
                <span className="font-mono text-sm font-bold text-white tabular-nums">{loading ? "-" : urgentCount}</span>
              </div>
              <div className="flex items-center justify-between group/row hover:bg-stone-800 p-1 rounded transition-colors">
                <span className="flex items-center gap-2 text-sm text-stone-300"><span className="h-2 w-2 rounded-full bg-amber-400" /> Moderate</span>
                <span className="font-mono text-sm font-bold text-white tabular-nums">{loading ? "-" : moderateCount}</span>
              </div>
              <div className="flex items-center justify-between group/row hover:bg-stone-800 p-1 rounded transition-colors">
                <span className="flex items-center gap-2 text-sm text-stone-300"><span className="h-2 w-2 rounded-full bg-blue-400" /> Routine</span>
        ...
        {/* Live Activity Feed */}
        <div className="rounded-xl border border-stone-200 bg-white shadow-md flex flex-col h-[400px]">
          <div className="border-b border-stone-100 p-4 flex items-center justify-between bg-stone-50/80 rounded-t-xl">
            <h2 className="text-[11px] font-bold uppercase tracking-wider text-stone-700 flex items-center gap-2"><Zap className="h-3.5 w-3.5 text-rose-600" /> Live Feed</h2>
            <Link href="/dashboard/audit" className="text-[10px] font-bold text-rose-600 hover:text-rose-700">View All</Link>
          </div>
          <div className="flex-1 p-4 overflow-y-auto relative">
            <div className="absolute inset-y-0 left-6 w-[2px] bg-stone-100" />
            {loading ? (
              <div className="space-y-4">
                {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-10 animate-pulse rounded bg-stone-100" />)}
              </div>
            ) : logs.length ? (
              <div className="space-y-4">
                {logs.map((log) => (
                  <div key={log.id} className="relative flex gap-3 animate-page-in">
                    <div className="relative z-10 flex h-5 w-5 mt-0.5 shrink-0 items-center justify-center rounded-full border-2 border-white bg-stone-200 text-stone-500 shadow-sm">
                      <FileText className="h-2.5 w-2.5" />
                    </div>
                    <div className="flex-1 pb-1">
                      <p className="text-[11px] font-semibold text-stone-900 leading-tight">{log.event_type.replace(/_/g, " ")}</p>
                      <p className="text-[9px] font-mono text-stone-400 mt-0.5">{new Date(log.created_at).toLocaleTimeString()}</p>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8 text-xs text-stone-400 relative z-10 bg-white">No recent activity.</div>
            )}
          </div>
        </div>
      </div>
