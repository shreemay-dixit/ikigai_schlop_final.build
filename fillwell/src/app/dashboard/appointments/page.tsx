  ...
  const columns: ColumnDef<Apt>[] = useMemo(() => [
    {
      ...
      cell: ({ row }) => (
        <div><p className="font-semibold text-slate-900">{row.original.patient_name}</p><p className="text-[11px] text-slate-500 font-mono">{row.original.patient_phone}</p></div>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return <div><p className="text-slate-800 font-medium">{d.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</p><p className="text-[11px] text-slate-500">{d.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}</p></div>;
      },
    },
    { accessorKey: "service_type", header: "Service", cell: ({ row }) => <span className="text-slate-600">{row.original.service_type}</span> },
  ...
  return (
    <div className="max-w-6xl space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold tracking-tight text-slate-900"><Calendar className="h-6 w-6 text-indigo-600" />Appointments</h1>
          <p className="text-sm text-slate-500">Manage schedule. Cancel to trigger automated recovery.</p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <tr><td colSpan={columns.length} className="py-12 text-center text-slate-400">No appointments found. Seed demo data from the Overview page.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        <div className="border-t border-slate-100 bg-slate-50/60 px-4 py-2.5 text-xs text-slate-500">{table.getRowModel().rows.length} records</div>
      </div>
    </div>
  );
}
