...
export function ConversationDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/70 backdrop-blur-sm transition-opacity">
      <div className="w-full max-w-xl bg-stone-950 border-l border-stone-800 h-full flex flex-col shadow-2xl animate-in slide-in-from-right duration-200">
        {/* Drawer Header */}
        <div className="flex items-center justify-between p-5 border-b border-stone-800 bg-stone-900/60">
        ...
        {/* View Switcher */}
        <div className="flex border-b border-stone-800 bg-stone-900/30 px-5 pt-2">
          <button
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {loading ? (
            <div className="space-y-3 animate-pulse">
              <div className="h-16 bg-stone-900 rounded-xl"></div>
              <div className="h-24 bg-stone-900 rounded-xl"></div>
              <div className="h-20 bg-stone-900 rounded-xl"></div>
            </div>
          ) : logs.length === 0 ? (
            ...
            <div className="space-y-4">
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="rounded-xl border border-stone-800 bg-stone-900/50 p-4 space-y-3"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-mono font-bold px-2 py-0.5 rounded bg-rose-500/10 text-rose-400 border border-rose-500/20 uppercase">
                  ...
                  {log.payload.raw_message && (
                    <div className="flex gap-2.5 items-start bg-stone-950/80 p-3 rounded-lg border border-stone-800/80">
            ...
            <div className="space-y-3">
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="rounded-xl border border-stone-800 bg-stone-950 p-4 font-mono text-[11px] text-emerald-400 overflow-x-auto"
                >
